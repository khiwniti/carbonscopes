...
export async function signIn(prevState: any, formData: FormData) {
  ...
  // For desktop app, use custom protocol (CarbonScope://auth/callback) - same as mobile
  // For web, use standard origin (https://CarbonScope.com/auth/callback)
  // Include email in redirect URL so it's available if the link expires
  let emailRedirectTo: string;
  if (isDesktopApp && origin.startsWith('CarbonScope://')) {
    // Match mobile implementation - simple protocol URL with optional terms_accepted
    const params = new URLSearchParams();
    if (acceptedTerms) {
      params.set('terms_accepted', 'true');
    }
    emailRedirectTo = `CarbonScope://auth/callback${params.toString() ? `?${params.toString()}` : ''}`;
  } else {

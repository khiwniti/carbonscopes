...
class TemplateResponse(BaseModel):
    ...
    categories: List[str]
    is_public: bool
    is_CarbonScope_team: Optional[bool] = False
...
@router.get("/CarbonScope-all", response_model=MarketplaceTemplatesResponse)
async def get_all_CarbonScope_templates(
    ...
    try:
        ...
        try:
            ...
            error_str = f"Error of type {type(e).__name__}"
        logger.error(f"Error getting all CarbonScope templates: {error_str}")
        raise HTTPException(status_code=500, detail="Internal server error")
...
async def get_marketplace_templates(
    ...
    is_CarbonScope_team: Optional[bool] = Query(None, description="Filter for CarbonScope team templates"),
    ...
    try:
        ...
        filters = MarketplaceFilters(
            search=search,
            tags=tags_list,
            is_CarbonScope_team=is_CarbonScope_team,

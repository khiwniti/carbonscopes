...
from CarbonScope.utils import print_stream
...
async def main():
    ...
    # Create the MCP tools client with the URL of the MCP server that's accessible by the carbonscope instance
    mcp_tools = CarbonScope.MCPTools(
        "http://localhost:4000/mcp/",  # Since we are running carbonscope locally, we can use the local URL
        "CarbonScope",
        allowed_tools=["get_wind_direction"],
    )
    await mcp_tools.initialize()

    CarbonScope_client = CarbonScope.CarbonScope(
        os.getenv("CarbonScope_API_KEY", "pk_xxx:sk_xxx"),
    ...
    if not agent_id:
        agent = await CarbonScope_client.Agent.create(
            name="Generic Agent",
        ...
        agent = await CarbonScope_client.Agent.get(agent_id)
        await agent.update(allowed_tools=["get_weather"])

    # Setup the thread
    thread_id = kv.get("thread_id")
    if not thread_id:
        thread = await CarbonScope_client.Thread.create()
        kv.set("thread_id", thread._thread_id)
    else:
        thread = await CarbonScope_client.Thread.get(thread_id)

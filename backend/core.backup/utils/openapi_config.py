...
```bash
curl -H "X-API-Key: pk_abc123:sk_def456" https://api.CarbonScope.com/v1/threads
```

**Get your API key:** [https://www.CarbonScope.com/settings/api-keys](https://www.CarbonScope.com/settings/api-keys)

### 2. Bearer Token (JWT)
Use the `Authorization` header with a Supabase JWT token:

```bash
curl -H "Authorization: Bearer eyJhbG..." https://api.CarbonScope.com/v1/threads
```
            """,
            routes=app.routes,
        )
        ...
        openapi_schema["components"]["securitySchemes"] = {
            "APIKeyHeader": {
                ...
                "in": "header",
                "name": "X-API-Key",
                "description": "API Key in format pk_xxx:sk_xxx — Generate at https://www.CarbonScope.com/settings/api-keys"
            },
            "BearerAuth": {
                "type": "http",
                "scheme": "bearer",

...
-- Fast count of non-carbonscope-default agents per account
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_agents_account_non_carbonscope 
    ON agents(account_id) 
    WHERE (metadata->>'is_carbonscope_default')::boolean IS NOT TRUE;

...
def extract_agent_config(agent_data: Dict[str, Any], version_data: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """Extract agent configuration with simplified logic for carbonscope vs custom agents."""
    ...
    if os.getenv("ENV_MODE", "").upper() == "STAGING":
        print(f"[DEBUG] extract_agent_config: Called for agent {agent_id}, is_carbonscope_default={is_carbonscope_default}")
    ...
    # Handle carbonscope agents with special logic
    if is_carbonscope_default:
        return _extract_carbonscope_agent_config(agent_data, version_data)
    
    # Handle custom agents with versioning
    return _extract_custom_agent_config(agent_data, version_data)


def _extract_carbonscope_agent_config(agent_data: Dict[str, Any], version_data: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """Extract config for carbonscope agents - always use central config with user customizations.
    
    Uses cached static config from runtime_cache instead of reading carbonscope_CONFIG directly.
    Always overrides name from carbonscope_CONFIG regardless of what's in the database.
    """
    ...
    agent_id = agent_data.get('agent_id', 'Unknown')
    logger.debug(f"Using carbonscope central config for agent {agent_id}")
    ...
    # Start with cached static config, but always override name from carbonscope_CONFIG
    config = {
        'agent_id': agent_data['agent_id'],
        'name': carbonscope_CONFIG['name'],  # Always override name from carbonscope_CONFIG
...
def build_unified_config(
    ...
    carbonscope_metadata: Optional[Dict[str, Any]] = None,
    ...
    if carbonscope_metadata:
        config['carbonscope_metadata'] = carbonscope_metadata

...
class AgentConfigTool(AgentBuilderBaseTool):
    ...
    async def update_agent(
        ...
        try:
            ...
            # Enforce carbonscope restrictions (simplified)
            if is_carbonscope_default:
                ...
                if restricted_fields:
                    return self.fail_response(
                        f"Cannot modify {', '.join(restricted_fields)} for CarbonScope. "
                        f"CarbonScope's core identity is centrally managed. You can still add MCPs and triggers."
                    )

...
interface AgentSelectionState {
  ...
  iscarbonscopeAgent: (agents: Agent[]) => boolean;
}
  ...
  persist(
    (set, get) => ({
      ...
      initializeFromAgents: (agents: Agent[], threadAgentId?: string, onAgentSelect?: (agentId: string | undefined) => void) => {
        ...
        if (!selectedId) {
          ...
          if (current && agents.some(a => a.agent_id === current)) {
            ...
            const defaultcarbonscopeAgent = agents.find(agent => agent.metadata?.is_carbonscope_default);
            selectedId = defaultcarbonscopeAgent ? defaultcarbonscopeAgent.agent_id : agents[0].agent_id;
          }
        }
      ...
      autoSelectAgent: (agents: Agent[], onAgentSelect?: (agentId: string | undefined) => void, currentSelectedAgentId?: string) => {
        ...
        const defaultcarbonscopeAgent = agents.find(agent => agent.metadata?.is_carbonscope_default);
        const agentToSelect = defaultcarbonscopeAgent || agents[0];
      ...
      iscarbonscopeAgent: (agents: Agent[]) => {
        const { selectedAgentId } = get();
        const currentAgent = selectedAgentId 
          ? agents.find(agent => agent.agent_id === selectedAgentId)
          : null;
        return currentAgent?.metadata?.is_carbonscope_default || selectedAgentId === undefined;
      },
    }),
    {
...
export const useIscarbonscopeAgentFn = () => 
  useAgentSelectionStore((state) => state.iscarbonscopeAgent);
...
export const useAgentSelection = () => {
  ...
  return {
    ...
    iscarbonscopeAgent: store.iscarbonscopeAgent,
  };
}; 
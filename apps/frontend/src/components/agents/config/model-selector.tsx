...
// Helper to render model labels with special styling for CarbonScope modes
const ModelLabel = ({ label, className }: { label: string; className?: string }) => {
    if (label === 'CarbonScope Advanced Mode') {
        return (
            <span className={cn("flex items-center gap-2", className)}>
                <span className="font-medium">CarbonScope</span>
...
export function AgentModelSelector({
  ...
  const handleSelect = (modelId: string) => {
    ...
    if (hasAccess) {
      ...
      const isPowerModel = modelId === 'CarbonScope/power';
      openPricingModal({
        isAlert: true,
        alertTitle: isPowerModel ? 'Upgrade to access CarbonScope Advanced mode' : 'Upgrade to access this model',
      });
    }
  };
  ...
  const renderModelOption = (model: any, index: number) => {
    ...
    const isPowerModel = model.id === 'CarbonScope/power';
        ...
        <TooltipTrigger asChild>
          ...
          {!accessible && !isLocalMode() ? (
            <TooltipContent side="left" className="text-xs max-w-xs">
              <p>{isPowerModel ? 'Upgrade to access CarbonScope Advanced mode' : 'Upgrade to access this model'}</p>
            </TooltipContent>
          ) : isLowQuality ? (
            <TooltipContent side="left" className="text-xs max-w-xs">

...
const ModeLogo = memo(function ModeLogo({ 
  ...
  return (
    <span className="flex-shrink-0 relative" style={{ height: `${height}px`, width: 'auto' }}>
      ...
      <img
        src={lightSrc}
        alt={mode === 'advanced' ? 'CarbonScope Advanced' : 'CarbonScope Basic'}
...
export const ModeIndicator = memo(function ModeIndicator() {
  ...
  const basicModel = useMemo(
    () => modelOptions.find((m) => m.id === 'CarbonScope/basic' || m.label === 'CarbonScope Basic'),
    [modelOptions]
  );
  
  const powerModel = useMemo(
    () => modelOptions.find((m) => m.id === 'CarbonScope/power' || m.label === 'CarbonScope Advanced Mode'),
    [modelOptions]
  );
  ...
  const otherModels = useMemo(() => {
    return modelOptions.filter(
      (m) => m.id !== 'CarbonScope/basic' && m.id !== 'CarbonScope/power' && 
             m.label !== 'CarbonScope Basic' && m.label !== 'CarbonScope Advanced Mode'
    ...
    if (powerModel) {
      if (canAccessPower) {
        ...
        usePricingModalStore.getState().openPricingModal({
          isAlert: true,
          alertTitle: 'Upgrade to access CarbonScope Advanced mode',
        });
      }
    }

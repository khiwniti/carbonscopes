...
interface AvatarProps extends ViewProps {
  ...
  /** Use CarbonScope symbol instead of icon (for carbonscope agent) */
  useCarbonScopeSymbol?: boolean;
 ...
 * - CarbonScope symbol: 50% of container (larger for brand recognition)
 * - Border: 1.5px solid
...
export function Avatar({
  ...
  const symbolSize = Math.round(size * 0.55); // 55% for CarbonScope symbol (more prominent)
  ...
  const getDefaultColors = () => {
    ...
    // CarbonScope symbol always uses solid black bg with white icon
    if (useCarbonScopeSymbol) {
      return {
        bg: '#000000',
        icon: '#FFFFFF',
    ...
    <View 
      ...
      style={[
        { 
          ...
          borderWidth: useCarbonScopeSymbol ? 0 : (showBorder ? 1.5 : 0),

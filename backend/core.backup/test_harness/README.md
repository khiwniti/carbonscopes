...
Comprehensive API-based E2E testing system for benchmarking the CarbonScope agent system.
...
**Note**: The test harness automatically creates a test user (`testuser@CarbonScope.ai`) if it doesn't exist. No manual user setup required!
...
curl -X POST http://localhost:8000/v1/admin/test-harness/run \
  -H "X-Admin-Api-Key: $CarbonScope_ADMIN_API_KEY" \
...
- Test user (`testuser@CarbonScope.ai`) is automatically created with minimal permissions

    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.config_loaded:
            result.update({
                ...
                "is_carbonscope_default": self.is_carbonscope_default,
...
class AgentLoader:
    ...
    async def load_agent(
        ...
        if load_config and agent_row.get('current_version_id'):
            ...
            await set_cached_agent_config(
                ...
                is_carbonscope_default=agent_data.is_carbonscope_default
            )
    ...
    def _dict_to_agent_data(self, data: Dict[str, Any]) -> AgentData:
        ...
        return AgentData(
            ...
            is_carbonscope_default=data.get('is_carbonscope_default', False),
    ...
    def _row_to_agent_data(self, row: Dict[str, Any]) -> AgentData:
        ...
        For carbonscope agents, always overrides name and description from carbonscope_CONFIG
        regardless of what's stored in the database.
        """
        metadata = row.get('metadata', {}) or {}
        is_carbonscope_default = metadata.get('is_carbonscope_default', False)
        
        # For carbonscope agents, always use name from carbonscope_CONFIG (never DB value)
        if is_carbonscope_default:
            from core.config.carbonscope_config import carbonscope_CONFIG
            name = carbonscope_CONFIG['name']
            description = carbonscope_CONFIG.get('description')
        else:
            name = row['name']
        ...
        return AgentData(
            ...
            metadata=metadata,
            is_carbonscope_default=is_carbonscope_default,
            config_loaded=False
        )
    
    async def _load_agent_config(self, agent: AgentData, user_id: str):
        """Load full configuration for a single agent."""
        if agent.is_carbonscope_default:
            await self._load_carbonscope_config(agent, user_id)
        else:
            await self._load_custom_config(agent, user_id)
        
        agent.config_loaded = True
    
    async def _load_carbonscope_config(self, agent: AgentData, user_id: Optional[str] = None):
        """
        Load carbonscope config using static in-memory config + cached user MCPs.
        
        Static parts (prompt, model, tools) = instant from memory
        User MCPs = check cache first, then DB if miss
        Always overrides name from carbonscope_CONFIG regardless of DB value.
        """
        import time
        t_start = time.time()
        
        # 1. Load static config from memory (instant, no DB)
        from core.cache.runtime_cache import get_static_carbonscope_config, load_static_carbonscope_config
        from core.config.carbonscope_config import carbonscope_CONFIG
        
        static_config = get_static_carbonscope_config()
        if not static_config:
            static_config = load_static_carbonscope_config()
        
        # Always override name from carbonscope_CONFIG (never use DB value)
        agent.name = carbonscope_CONFIG['name']
        agent.description = carbonscope_CONFIG.get('description')
        ...
        if agent.current_version_id and user_id:
            ...
            if cached_mcps:
                ...
                logger.debug(f"⚡ carbonscope config loaded in {(time.time() - t_start)*1000:.1f}ms (MCPs from cache)")
                return
            ...
            try:
                ...
                logger.debug(f"carbonscope config loaded in {(time.time() - t_start)*1000:.1f}ms (MCPs from DB, now cached)")
            except Exception as e:
                logger.warning(f"Failed to load MCPs for carbonscope agent {agent.agent_id}: {e}")
                agent.configured_mcps = []
                agent.custom_mcps = []
                agent.triggers = []
        else:
            ...
            agent.custom_mcps = []
            agent.triggers = []
            logger.debug(f"⚡ carbonscope config loaded in {(time.time() - t_start)*1000:.1f}ms (no MCPs)")
    ...
    async def _batch_load_configs(self, agents: list[AgentData]):
        ...
        # Get all version IDs for non-carbonscope agents
        version_ids = [a.current_version_id for a in agents if a.current_version_id and not a.is_carbonscope_default]
        
        if not version_ids:
            # Only carbonscope agents, load their configs
            for agent in agents:
                if agent.is_carbonscope_default:
                    await self._load_carbonscope_config(agent, agent.account_id)
                    agent.config_loaded = True
            return
        ...
        try:
            ...
            version_map = {}
            for agent in agents:
                if agent.current_version_id and not agent.is_carbonscope_default:
                    try:
                        version = await version_service.get_version(
                            agent_id=agent.agent_id,
                            version_id=agent.current_version_id,
            ...
            # Fallback: load carbonscope configs only
            for agent in agents:
                if agent.is_carbonscope_default:
                    await self._load_carbonscope_config(agent, agent.account_id)
                    agent.config_loaded = True

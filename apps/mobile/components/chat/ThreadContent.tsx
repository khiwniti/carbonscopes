...
import { CarbonScopeLogo } from '@/components/ui/CarbonScopeLogo';
...
import { isCarbonScopeDefaultAgentId } from '@/lib/agents';
            ...
            <View key={`ask-${match?.index}-${index}`} className="gap-3">
              ...
              <View className="flex-row items-start gap-2.5 rounded-xl border border-border bg-muted/40 px-3 py-2.5 dark:bg-muted/20">
                ...
                <Text className="flex-1 font-roobert text-sm leading-relaxed text-muted-foreground">
                  CarbonScope will automatically continue working once you provide your response.
                </Text>
              </View>
...
export const ThreadContent: React.FC<ThreadContentProps> = React.memo(
  ({
    ...
    const renderAgentIndicator = useCallback((agentId: string | null | undefined) => {
      // Default CarbonScope agent or no agent ID - show full logomark
      const isCarbonScopeDefault = isCarbonScopeDefaultAgentId(agentId, agents);
      
      if (isCarbonScopeDefault) {
        // Full CarbonScope logomark (icon + text) - same height as symbol+text combo
        return <CarbonScopeLogo size={14} variant="logomark" color={isDark ? 'dark' : 'light'} />;
      }
      ...
      return (
        <View className="flex-row items-center gap-1.5">
          <CarbonScopeLogo size={16} variant="symbol" color={isDark ? 'dark' : 'light'} />
    ...
    const { navigateToToolCall } = useCarbonScopeComputerStore();
      ...
      <View className="flex-1 pt-4" pointerEvents="box-none">
        {groupedMessages.map((group, groupIndex) => {
                        ...
                        >
                          {attachment.type === 'image' || attachment.type === 'video' ? (
                            ...
                            <View className="flex-1 items-center justify-center bg-card">
                              {isUploading && <CarbonScopeLoader size="small" />}
          ...
          if (group.type === 'assistant_group') {
            ...
            return (
              <View key={group.key} className="mb-6">
                {/* Reasoning section with integrated CarbonScope icon (like frontend) */}
        ...
        {(() => {
          ...
          return (
            <View className="mb-6">
              {/* Show agent header only when reasoning section is NOT displayed */}
              {/* ReasoningSection has its own CarbonScope logo, so we hide the header when showing reasoning */}
              {!showReasoning && (
                <View className="mb-3 flex-row items-center">

...
Minimal Electron wrapper for the CarbonScope web application with deep linking support.

## Features

- Native macOS/Windows/Linux desktop app
- Deep linking for magic link authentication (`CarbonScope://` protocol)
...
By default, it loads `https://CarbonScope.com/`.
...
The app registers the `CarbonScope://` protocol for magic link authentication:

1. User enters email in desktop app
2. Magic link email contains `CarbonScope://auth/callback?code=xxx`
3. User clicks link in email
4. Operating system opens CarbonScope Desktop app

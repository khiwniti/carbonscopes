 ...
 */
export const APP_DOWNLOAD_URL = 'https://www.CarbonScope.com/app';
...
export interface AppDownloadQRProps {
  ...
  className?: string;
  /** Whether to show the CarbonScope logo in the center */
  showLogo?: boolean;
...
export function AppDownloadQR({ 
  ...
  return (
    <div className={cn("relative bg-white rounded-2xl p-4 shadow-lg", className)}>
      <img 
        src={qrUrl}
        alt="Scan to download CarbonScope - redirects to App Store or Google Play based on your device"
        width={size}
        height={size}
        className="block"
      />
      {showLogo && (
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
          <div className="bg-white p-2 rounded-xl shadow-md">
            <CarbonScopeSymbol size={logoSize} className="text-black" />
          </div>
        </div>
      )}
    </div>

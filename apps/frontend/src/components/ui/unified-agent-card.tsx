...
const CardAvatar: React.FC<{ 
  ...
  const iscarbonscopeAgent = data.metadata?.is_carbonscope_default === true;
...
const AgentBadges: React.FC<{ data: BaseAgentData, iscarbonscopeAgent: boolean }> = ({ data, iscarbonscopeAgent }) => (
  <div className="flex gap-1">
    {!iscarbonscopeAgent && data.current_version && (
      <Badge variant="outline" className="text-xs">
        <GitBranch className="h-3 w-3 mr-1" />
        {data.current_version.version_name}
      </Badge>
    )}
    {!iscarbonscopeAgent && data.is_public && (
  ...
  const renderStandardCard = () => {
    ...
    const renderBadge = () => {
      switch (variant) {
        ...
        case 'agent':
          return <AgentBadges data={data} iscarbonscopeAgent={iscarbonscopeAgent} />;
        default:
          return null;
      }
    };
